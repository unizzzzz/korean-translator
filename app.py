...
def translate_with_ai(text, direction, api_key):
    ...
    # 【修復2】加入「嚴格指令」，逼迫 AI 只輸出句子，不准碎碎念
    if direction == "韓文 ➡️ 中文":
        prompt = f"""你是一個專業的韓文翻譯。請將以下韓文翻譯成流暢的台灣繁體中文。
        【嚴格指令】只輸出翻譯結果，絕對不要加上任何解釋、拼音、引言或多餘文字。
        需要翻譯的文字：\n{text}"""
    else:
        prompt = f"""你是一個精通韓文的台灣人。請將以下中文翻譯成自然道地的韓文。請使用適當的敬語。
        【嚴格指令】只輸出翻譯後的韓文句子，絕對不要加上任何解釋、羅馬拼音、引言或多餘文字。
        需要翻譯的文字：\n{text}"""
...
spoken_text = speech_to_text(
    ...
    stop_prompt="停止錄音並輸出",
    just_once=True,
    key=f"stt_{direction}"
)

# 【修復1】只要麥克風有收到文字，就把它強行存進記憶體裡
if spoken_text:
    st.session_state.text_input_area = spoken_text

# 這裡綁定記憶體 (key="text_input_area")，這樣按按鈕時文字才不會消失
text_to_translate = st.text_area("或手動修改/輸入文字：", key="text_input_area", height=120)
